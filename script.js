...
function generateCard() {
    ...
    const fullURL = `${window.location.origin}${window.location.pathname}?name=${encodeURIComponent(senderName)}&message=${encodeURIComponent(greetingMessage)}`;
    ...
    .then(data => {
      document.getElementById('personalizedLink').value = data.data.tiny_url;
    })
    .catch(() => {
      document.getElementById('personalizedLink').value = fullURL; // fallback to full URL
    });
  }
  ...
  function startCountdown() {
    const eidDate = new Date("2025-03-31T18:30:00");
